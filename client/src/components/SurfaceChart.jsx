...
export default function SurfaceChart({ filterParams, tour }) {
  ...
  const tourWinLabel = tour === 'WTA' ? 'WTA 1000/500' : tour === 'ATP' ? 'Masters 1000' : 'Tour Titles';
  const tourRuLabel = tour === 'WTA' ? 'WTA 1000/500 RU' : tour === 'ATP' ? 'Masters 1000 RU' : 'Tour RU';
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Wins bar chart */}
        <div className="card">
          <h3 className="text-sm font-semibold font-display text-slate-300 mb-4">Titles — Grand Slam vs {tourWinLabel}</h3>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={top10} layout="vertical" margin={{ left: 100, right: 20 }}>
              ...
              />
              <Legend />
              <Bar dataKey="slam_wins" name="Grand Slam" stackId="a" fill="#F59E0B" />
              <Bar dataKey="tour_wins" name={tourWinLabel} stackId="a" fill="#10B981" radius={[0,4,4,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Runner-up bar chart */}
        <div className="card">
          <h3 className="text-sm font-semibold font-display text-slate-300 mb-4">Runner-ups — Grand Slam vs {tourWinLabel}</h3>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={top10} layout="vertical" margin={{ left: 100, right: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis type="number" tick={{ fill: '#94a3b8', fontSize: 11 }} />
              ...
              />
              <Legend />
              <Bar dataKey="slam_ru" name="Grand Slam RU" stackId="a" fill="#F59E0B" fillOpacity={0.5} />
              <Bar dataKey="tour_ru" name={tourRuLabel} stackId="a" fill="#10B981" fillOpacity={0.5} radius={[0,4,4,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      {/* Finals win rate */}
      <div className="card">
        <h3 className="text-sm font-semibold font-display text-slate-300 mb-4">Finals Win Rate (top 15 by finals played)</h3>
